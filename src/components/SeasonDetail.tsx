    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-pink-50 to-slate-100 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950">
      ...
      <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 ${isMobile ? 'px-4 py-4' : ''}`}>
        ...
        <div className="mb-6">
          <Link
            to={`/tv/${id}`}
            className="inline-flex items-center space-x-2 text-[var(--grad-from)] dark:text-[var(--grad-from)] hover:text-[var(--grad-to)] dark:hover:text-pink-300 transition-colors"
          >
              ...
              <div className="flex items-start justify-between">
                {isMobile && (
                  <div className="flex items-start space-x-4 flex-1">
                    ...
                    <div className="flex-1">
                      ...
                      <h2 className="text-lg font-semibold text-[var(--grad-from)] dark:text-[var(--grad-from)]">
                        {season.name}
                      </h2>
                ...
                {!isMobile && (
                  <div className="flex-1">
                    ...
                    <h2 className="text-2xl font-semibold text-[var(--grad-from)] dark:text-[var(--grad-from)] mb-4">
                      {season.name}
                    </h2>
                    <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">
                      {season.episodes?.length || 0} {t.episodes} · {getFormattedYearRange(season.episodes)}
                  ...
                  <div className="flex items-center justify-between">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <div className="flex items-center">
                            <span className={`bg-gradient-to-r from-[var(--grad-from)] to-[var(--grad-to)] text-white px-2 py-1 rounded-full font-semibold ${isMobile ? 'text-xs' : 'text-sm'}`}>
                              {episode.episode_number}
                            </span>
                            <Link
                              to={`/tv/${id}/season/${episode.season_number}/episode/${episode.episode_number}`}
                              className={`font-semibold text-gray-900 dark:text-white hover:text-[var(--grad-from)] dark:hover:text-[var(--grad-from)] transition-colors ml-2 ${isMobile ? 'text-sm truncate' : 'text-lg'}`}
                            >
                              {episode.name}
                            </Link>
                          </div>
                    ...
                    <div className="ml-4 flex-shrink-0 flex items-center space-x-2">
                      <button
                        ...
                        className={`bg-gradient-to-r from-[var(--grad-from)] to-[var(--grad-to)] text-white font-semibold hover:from-pink-600 hover:to-purple-700 transition-colors flex items-center space-x-1 shadow-lg ${isMobile ? 'px-2 py-1 rounded-lg text-xs' : 'px-4 py-2 rounded-xl text-sm'}`}
                      >
